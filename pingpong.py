# Implementation of classic arcade game Pong

import simplegui
import random

# initialize globals - pos and vel encode vertical info for paddles
WIDTH = 600
HEIGHT = 400       
BALL_RADIUS = 20
PAD_WIDTH = 10
PAD_HEIGHT = 80
HALF_PAD_WIDTH = PAD_WIDTH / 2
HALF_PAD_HEIGHT = PAD_HEIGHT / 2
LEFT = False
RIGHT = True
ball_vel =[0,0]

# initialize ball_pos and ball_vel for new bal in middle of table
# if direction is RIGHT, the ball's velocity is upper right, else upper left
def spawn_ball(direction):
    global ball_pos, ball_vel # these are vectors stored as lists


# define event handlers
def new_game():
    global paddle1_pos, paddle2_pos, paddle1_vel, paddle2_vel  # these are numbers
    global score1, score2  # these are ints
    #adding my line
    global ball_pos,ball_vel
    paddle1_pos = HEIGHT/2
    paddle2_pos = HEIGHT/2
    paddle1_vel = 0
    paddle2_vel = 0
    ball_pos = [WIDTH/2 , HEIGHT/2]
    score1=0
    score2=0
    LEFT = False
    RIGHT = True
    ball_vel =[0,0]

def draw(canvas):
    global score1, score2, paddle1_pos, paddle2_pos, ball_pos, ball_vel 
    #my declarion
    global RIGHT , LEFT
 
        
    # draw mid line and gutters
    canvas.draw_line([WIDTH / 2, 0],[WIDTH / 2, HEIGHT], 1, "White")
    canvas.draw_line([PAD_WIDTH, 0],[PAD_WIDTH, HEIGHT], 1, "White")
    canvas.draw_line([WIDTH - PAD_WIDTH, 0],[WIDTH - PAD_WIDTH, HEIGHT], 1, "White")
        
    # update ball
    if (RIGHT == True ):
        ball_vel= [4,4]
        RIGHT = False
    elif (LEFT == True):
        ball_vel = [-4,4]
        LEFT = False
    ball_pos[0] += ball_vel[0]
    ball_pos[1] += ball_vel[1]
    if (ball_pos[1]>(HEIGHT-BALL_RADIUS) and ball_vel[0] > 0 and ball_vel[1] > 0):
        ball_vel[1]=-ball_vel[1]
    elif (ball_pos[1]>(HEIGHT-BALL_RADIUS) and ball_vel[0] <0 and ball_vel[1] >0):
        ball_vel[1]=-ball_vel[1]
    elif (ball_pos[1]<= BALL_RADIUS and ball_vel[0] >0 and ball_vel[1] <0):
        ball_vel[1]=-ball_vel[1]
    elif (ball_pos[1]<= BALL_RADIUS and ball_vel[0] <0 and ball_vel[1] <0):
        ball_vel[1]=-ball_vel[1]

    # draw ball
    canvas.draw_circle(ball_pos , BALL_RADIUS , 1,"White","White")
    
    
    # update paddle's vertical position, keep paddle on the screen
    
    paddle1_pos += paddle1_vel
    paddle2_pos += paddle2_vel
    if ((paddle1_pos-PAD_HEIGHT/2 ) < 0 or (paddle1_pos+PAD_HEIGHT/2)>HEIGHT):
        paddle1_pos -= paddle1_vel
    if ((paddle2_pos-PAD_HEIGHT/2 ) < 0 or (paddle2_pos+PAD_HEIGHT/2) > HEIGHT):
        paddle2_pos -= paddle2_vel
    
    # draw paddles
    canvas.draw_line([PAD_WIDTH/2 , (paddle1_pos-PAD_HEIGHT/2)],[PAD_WIDTH/2 , paddle1_pos+PAD_HEIGHT/2],PAD_WIDTH,"White")
    canvas.draw_line([WIDTH-PAD_WIDTH/2 , (paddle2_pos-PAD_HEIGHT/2)],[WIDTH-PAD_WIDTH/2 , paddle2_pos+PAD_HEIGHT/2],PAD_WIDTH,"White")
    
    # determine whether paddle and ball collide
    if (ball_pos[0]>=(WIDTH-PAD_WIDTH-BALL_RADIUS-1)):
        if ( ball_pos[1]>= (paddle2_pos-PAD_HEIGHT/2) and ball_pos[1] <= (paddle2_pos+PAD_HEIGHT/2)):
            ball_vel[0]=-ball_vel[0]
        else:
            ball_pos = [WIDTH/2 , HEIGHT/2]
            LEFT = True
            score1+=1
            
    elif (ball_pos[0]<=(PAD_WIDTH+BALL_RADIUS)):
        if ( ball_pos[1]>= (paddle1_pos-PAD_HEIGHT/2) and ball_pos[1] <= (paddle1_pos+PAD_HEIGHT/2)):
            ball_vel[0]=-ball_vel[0]
        else:
            ball_pos = [WIDTH/2 , HEIGHT/2]
            RIGHT = True
            score2+=1
            
    
    
    # draw scores
    canvas.draw_text(str(score1), (WIDTH/2-30, 20), 20, 'White')
    canvas.draw_text(str(score2), (WIDTH/2+30, 20), 20, 'White')
        
def keydown(key):
    global paddle1_vel, paddle2_vel
    vel = 8
    if key == simplegui.KEY_MAP['down']:
        paddle2_vel += vel
    elif key == simplegui.KEY_MAP['up']:
        paddle2_vel -= vel
    elif key == simplegui.KEY_MAP['W']:
        paddle1_vel -= vel
    elif key == simplegui.KEY_MAP['S']:
        paddle1_vel += vel
        
def keyup(key):
    global paddle1_vel, paddle2_vel
    if key == simplegui.KEY_MAP['down']:
        paddle2_vel = 0
    elif key == simplegui.KEY_MAP['up']:
        paddle2_vel = 0
    elif key == simplegui.KEY_MAP['W']:
        paddle1_vel = 0
    elif key == simplegui.KEY_MAP['S']:
        paddle1_vel =0

def button_handler():
    new_game()
def timer_handler():
    global ball_vel
    val=0.2
    if (ball_vel[0] > 0 and ball_vel[1] > 0):
        ball_vel[0]+=val
        ball_vel[1]+=val
    elif (ball_vel[0] > 0 and ball_vel[1] < 0):
        ball_vel[0]+=val
        ball_vel[1]-=val
    elif (ball_vel[0] < 0 and ball_vel[1] < 0):
        ball_vel[0]-=val
        ball_vel[1]-=val
    elif (ball_vel[0] < 0 and ball_vel[1] > 0):
        ball_vel[0]-=val
        ball_vel[1]+=val

# create frame
frame = simplegui.create_frame("Pong", WIDTH, HEIGHT)
frame.set_draw_handler(draw)
frame.set_keydown_handler(keydown)
frame.set_keyup_handler(keyup)
button1 = frame.add_button('Restart', button_handler)
timer = simplegui.create_timer(1000, timer_handler)


# start frame
new_game()
frame.start()
timer.start()
